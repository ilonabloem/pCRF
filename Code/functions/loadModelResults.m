...
allResults      = struct('allParams', structSize, ...
                         ...
                         'deconvParams', structSize, ...
                         'modelbasedParams', structSize, ...
...
for ii = 1:numTasks
    ...
    if exist(fullfile(outDir, saveName), 'file') > 0 && opts.compute == 0
        ...
        if strcmp(taskNames{ii}, 'JN2022Event')
            ...
            loadName    = 'voxelSelectionThresholds.mat';
            loadDir     = fullfile(dataRoot, 'JNeuro2022');
            assert(exist(fullfile(loadDir, loadName), 'file') > 0, 'Deconvolution analysis parameter estimates not found')
            data        = load(fullfile(loadDir, loadName)); 
            allResults(ii).voxThres = data.voxThres;
                        ...
                        case "nocross" % extract parameter estimates
                            ...
                            if contains(taskNames{ii}, 'JN2022Event')
                                ...
                                for p = 1:numel(paramOrder) 
                                    ...
                                    rho         = corr(modelParams(:,p), deconvParams(:,p), 'type', 'Spearman', 'Rows', 'pairwise');
                                ...
                                allResults(ii).deconvParams(s,:,roi)      = median(deconvParams, 1, "omitmissing");
                                allResults(ii).modelbasedParams(s,:,roi)  = median(modelParams, 1, "omitmissing");
